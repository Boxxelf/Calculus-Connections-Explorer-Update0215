document.addEventListener('DOMContentLoaded', () => {
    // Attach overlay handlers first so the modal is always dismissable, even if later code throws
    ...
    function updateCsSelectionSummary() {
        ...
        if (entries.length === 0) {
            csSelectionSummary.text('No ME topics selected. Pick one or more topics on the left.');
            return;
        }

        const total = entries.length;
        const preview = entries.slice(0, 4);
        let summary = `ME topics selected (${total}): ${preview.join(', ')}`;
        ...
        if (!hasCsSelection) {
            ...
            if (!calcListEmpty.empty()) {
                calcListEmpty
                    ...
                    .select('p')
                    .text('Choose a ME course or topic on the left to see the list of connected calculus ideas here.');
            }
        ...
        if (connections.length === 0) {
            ...
            if (!calcListEmpty.empty()) {
                calcListEmpty
                    .classed('hidden', false)
                    .select('p')
                    .text('No calculus topics are currently connected to the selected ME topics.');
            }
            updateActiveCalcListTopic(null);
            return;
        }
        ...
        if (selectedFilters.length === 0) {
            ...
            if (!added) {
                rationaleContent.append('p')
                    ...
                    .text('No rationales yet for this topic and your ME selection.');
            }
